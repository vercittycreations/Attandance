  ...
  const handleStatusChange = async (task, newStatus) => {
    ...
    try {
      ...
      if (newStatus === 'completed') {
        await logActivity(
          userProfile.uid, userProfile.name, 'task_completed',
          `${userProfile.name} completed task: "${task.title}"`,
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <button
            ...
            className={`card p-4 text-left transition-all ${filter === key ? 'border-brand-500/30 bg-brand-500/5' : 'hover:border-white/10'}`}
          >
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          ...
          return (
            <div key={task.id} className="card p-4 sm:p-5 hover:border-white/10 transition-all">
              <div className="flex items-start gap-3 sm:gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 flex-wrap">
                    <h3 className={`text-sm font-semibold ${task.status === 'completed' ? 'text-white/40 line-through' : 'text-white'}`}>
                      {task.title}
                    </h3>
