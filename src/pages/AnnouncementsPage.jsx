    ...
    try {
      if (editItem) {
        ...
        try {
          ...
          toast.success(
            `✅ Posted & notified ${others.length} employee${others.length !== 1 ? 's' : ''}!`
          );
        } catch {
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-white/40 mt-0.5">
            {isAdmin
              ? 'Post announcements — all employees get notified instantly'
          ...
          {!editItem && (
            <div className="flex items-start gap-2 p-3 bg-brand-500/10 border border-brand-500/20 rounded-xl">
              ...
              <p className="text-xs text-brand-300 leading-relaxed">
                All employees will receive a browser push notification
                and an in-app notification when you post.
              </p>
            </div>
          )}
