...
import {
  subscribeToAnnouncements, createAnnouncement,
  updateAnnouncement, deleteAnnouncement
} from '../services/announcementService';
import { logActivity } from '../services/activityService';
import Modal from '../components/common/Modal';
import { Megaphone, Plus, Pin, Edit2, Trash2, Calendar } from 'lucide-react';
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editItem) {
        ...
        await logActivity(currentUser.uid, userProfile?.name, 'task_assigned',
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-white/40 mt-0.5">
            {isAdmin ? 'Post and manage company announcements' : 'Stay updated with company news'}
          </p>
        </div>
        {isAdmin && (
          <button
      ...
      <div className="space-y-3">
        {announcements.length === 0 ? (
          ...
          <div key={item.id} className={`card p-5 transition-all ${item.isPinned ? 'border-brand-500/20 bg-brand-500/3' : ''}`}>
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-3">
                  ...
                  <span className="text-white/20 text-xs">by {item.createdByName}</span>
                </div>
              </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. New AI Tool Available"
              className="input"
            />
          </div>
          <div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={handleSubmit} disabled={submitting} className="btn-primary flex-1">
              {submitting ? 'Posting...' : editItem ? 'Update' : 'Post'}
            </button>
          </div>
        </div>
      </Modal>
