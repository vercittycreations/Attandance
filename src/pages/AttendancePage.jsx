...
import {
  ...
  getEmployeeAttendance, getSettings
} from '../services/attendanceService';
import { logActivity } from '../services/activityService';
import { calculateStreak, updateEmployeeStreak } from '../services/streakService';
...
export default function AttendancePage() {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      // Log activity
      await logActivity(
        userProfile.uid, userProfile.name, 'check_in',
        `${userProfile.name} checked in${result.status === 'late' ? ' (Late)' : ''}`,
        { status: result.status }
      );

      // Update streak
      const newHistory = await getEmployeeAttendance(userProfile.uid, 60);
      const newStreak = calculateStreak(newHistory);
      setStreak(newStreak);
      await updateEmployeeStreak(userProfile.uid, newStreak);
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      await logActivity(
        userProfile.uid, userProfile.name, 'check_out',
        `${userProfile.name} checked out`,
    ...
    <div className="space-y-5 animate-fade-in">
        ...
        <div className="flex flex-col items-start sm:items-end gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-white/40">
              Deadline: <span className="text-amber-400 font-medium">{deadline}</span>
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="card p-5">
          ...
          {!today ? (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-white/50">Not checked in</span>
            </div>
          ) : (
            <div className="space-y-2">
              <span className={`badge badge-${today.status} text-sm`}>
                {today.status.charAt(0).toUpperCase() + today.status.slice(1)}
              </span>
              {today.checkInTime && (
                <p className="text-xs text-white/40">
                  In: <span className="text-white/70">
                    {today.checkInTime?.toDate ? format(today.checkInTime.toDate(), 'HH:mm') : '--'}
                  </span>
                </p>
              )}
              {today.checkOutTime && (
                <p className="text-xs text-white/40">
                  Out: <span className="text-white/70">
                    {today.checkOutTime?.toDate ? format(today.checkOutTime.toDate(), 'HH:mm') : '--'}
                  </span>
                </p>
              )}
              {today.totalHours > 0 && (
                <p className="text-xs text-white/40">
                  Total: <span className="text-emerald-400 font-medium">{today.totalHours}h</span>
                </p>
              )}
            </div>
          )}
        ...
        <div className="card p-5 flex flex-col items-center justify-center gap-3">
          <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center ${checkedIn ? 'bg-emerald-500/15' : 'bg-white/5'}`}>
            <LogIn size={22} className={checkedIn ? 'text-emerald-400' : 'text-white/30'} />
          </div>
          <div className="text-center">
            <p className="text-sm font-medium text-white">Check In</p>
            {checkedIn && (
              <p className="text-xs text-emerald-400 flex items-center gap-1 mt-0.5 justify-center">
        ...
        {/* Check Out */}
        <div className="card p-5 flex flex-col items-center justify-center gap-3">
          <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center ${checkedOut ? 'bg-blue-500/15' : 'bg-white/5'}`}>
            <LogOut size={22} className={checkedOut ? 'text-blue-400' : 'text-white/30'} />
          </div>
          <div className="text-center">
            <p className="text-sm font-medium text-white">Check Out</p>
            {checkedOut && (
              <p className="text-xs text-blue-400 flex items-center gap-1 mt-0.5 justify-center">
                <CheckCircle size={11} /> Done
              </p>
            )}
          </div>
